...
public:
    ...
    virtual bool defragment(const fs::path& path) = 0;
};

class GenericDefragmenter : public Defragmenter {
public:
    bool defragment(const fs::path& path) override {
        ...
        try {
            ...
            // 1. Copy original file to a new temporary file
            // This implicitly tries to allocate contiguous blocks
            fs::copy_file(path, temp_path, fs::copy_options::overwrite_existing);
...
public:
    bool defragment(const fs::path& path) override {
        ...
        if (ioctl(fd, EXT4_IOC_MOVE_EXT, &me) < 0) {
            ...
            return generic.defragment(path);
        }
...
void process_path(const fs::path& path) {
    if (!fs::exists(path)) {
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        std::cout << "Universal Defragmenter v1.0" << std::endl;
        std::cout << "Usage: " << argv[0] << " <path>" << std::endl;
        return 1;
    }
    
    if (getuid() != 0) {
        std::cout << "Warning: This program usually requires root privileges for defragmentation." << std::endl;
    }
